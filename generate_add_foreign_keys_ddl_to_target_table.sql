...
JOIN 
    pg_class refcls ON con.confrelid = refcls.oid
JOIN 
    pg_namespace refnsp ON refcls.relnamespace = refnsp.oid
JOIN 
    pg_attribute att ON att.attnum = ANY(con.conkey) AND att.attrelid = con.conrelid
JOIN 
    pg_attribute refatt ON refatt.attnum = ANY(con.confkey) AND refatt.attrelid = con.confrelid
WHERE 
    refcls.relname = 'users'  -- replace with your table name
AND 
    refnsp.nspname = 'rideshare'  -- replace with your schema if different
GROUP BY 
    conname, nsp.nspname, cls.relname, refnsp.nspname, refcls.relname, confupdtype, confdeltype, att.attname, refatt.attname;
